...
logging.basicConfig(
    filename="unknown_primer.error.log", format="%(asctime)s:%(levelname)s:%(message)s"
)


def unknown_primer(
    f_primer, r_primer, hg_version, primer_name, max_size, min_perfect, min_good
):
    ...
    data = scrape_seq(
        primer_name, f_primer, r_primer, hg_version, max_size, min_perfect, min_good
    )
...
def scrape_seq(
    ...
    """
    url = (
        "https://genome.ucsc.edu/cgi-bin/hgPcr?hgsid=483"
        ...
        with open(args["input"], "r") as in_file:
            for line in in_file:
                try:
                    ...
                    logging.error(
                        "No protein coding transcript was found within the in-silico amplicon generated by {}".format(
                            primer_name
                        )
                    )
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Retrieve a primer pairs metadata from UCSC and Ensembl"
    )
    parser.add_argument("-p", "--primers", nargs=2, type=str, help="primer pair")
    parser.add_argument(
        "-i", "--input", type=str, help="a tab delimited file containing primer info"
    )
    parser.add_argument(
        "-hg",
        "--genome_version",
        type=str,
        help="human genome version (default=hg19)",
        default="hg19",
    )
    parser.add_argument(
        "-m",
        "--max_size",
        type=int,
        help="maximum in-silico amplicon size (default=4000)",
        default=4000,
    )
    parser.add_argument(
        "-mp",
        "--min_perfect",
        type=int,
        help="no. of bases that match exactly on 3end of primers (default=15)",
        default=15,
    )
    parser.add_argument(
        "-mg",
        "--min_good",
        type=int,
        help="no. of bases on 3end of primers where at least 2/3 bases match (default=15)",
        default=15,
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        help="output data to file name",
        default="output_primer_data.txt",
    )
    return parser
