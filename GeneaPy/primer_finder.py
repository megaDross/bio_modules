...
logging.basicConfig(
    filename="primer_finder.error.log", format="%(asctime)s:%(levelname)s:%(message)s"
)
...
def primer_finder(
    ...
    primers = extra_filters(primers, size, distance, gc, gene, exon, intron, hg)
    if output:
...
def extra_filters(primer, size, distance, gc, gene, exon, intron, hg):
    ...
    try:
        ...
        if distance:
            primer = primer[((primer.Dist_F > distance) & (primer.Dist_R > distance))]
        if gc:
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Find primers for a set of given variants."
    )
    parser.add_argument(
        "-d", "--database", type=str, required=True, help="primer database"
    )
    parser.add_argument(
        "-i", "--input", type=str, help="input file with variant positions"
    )
    parser.add_argument(
        "-v",
        "--variant",
        type=str,
        help="only amplicons that cover the given variant position",
    )
    parser.add_argument(
        "-s", "--size", type=int, help="maximum size of the resulting amplicon"
    )
    parser.add_argument(
        "-k",
        "--distance",
        type=int,
        help="minimum distance from forward/reverse primer",
    )
    parser.add_argument(
        "-c", "--gc", type=int, help="maximum GC%% of the resulting amplicon"
    )
    parser.add_argument(
        "-hg",
        "--genome_version",
        type=str,
        help="only amplicons produced in specific genome version",
    )
    parser.add_argument(
        "-g", "--gene", type=str, help="only amplicons lying within the given gene"
    )
    parser.add_argument(
        "-e", "--exon", type=str, help="only amplicons produced in a given exon numer"
    )
    parser.add_argument(
        "-n",
        "--intron",
        type=int,
        help="only amplicons lying within the given intron number",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        help="output the results of processing --input",
        default="output_primer_finder.txt",
    )
    return parser
