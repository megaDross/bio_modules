...
class LocusMetaData(object):
    ...
    def _sequence(self):
        ...
        seq = get_seq.get_seq(
            query, self.hg_version, self.genome, self.flank, self.flank, header=False
        )
        return seq
    ...
    @classmethod
    def from_position(
        cls, genomic_position, hg_version, flank=50, genome=None, gene_list=[]
    ):
    ...
    def __str__(self):
        ...
        gene_location = "{}:{}-{}".format(
            self.gene.contig, self.gene.start, self.gene.end
        )
        gene = "Gene\nname: {}\nid: {}\nloc: {}\ntype: {}\n\n".format(
