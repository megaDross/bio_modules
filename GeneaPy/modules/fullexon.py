...
class FullExon(Exon):
    ...
    def __init__(self, exon_id, contig, start, end, strand, gene_name, gene_id, position, number, exon):
    ...
    def from_pyexon(cls, Exon, position, number, exon):
        ...
        return cls(
            ...
            position, number, exon)
    
    def __str__(self):
        name = 'Exon' if self.exon else 'Intron'
        return name+"(exon_id=%s, gene_name=%s, contig=%s, start=%d, end=%s, position=%s, number=%s)" % (
            self.id, self.gene_name, self.contig, self.start, 
            self.end, self.position, self.number)
