...
class FullExon(Exon):
    ...
    @property
    def exon_no(self):
        exon_id, intron_num, exon_num = get_exon_intron_number.main(self.transcript_id, 
                                                          self.hg_version, 
                                                          self.position)
        if self.id != exon_id:
    ...
    @property
    def intron_no(self):
        exon_id, exon_num, intron_num = get_exon_intron_number.main(self.transcript_id, 
                                                          self.hg_version, 
                                                          self.position)
        return intron_num
    ...
    def from_pyexon(cls, exon, position, transcript_id, hg_version):
        ...
        return cls(exon.id, exon.contig, exon.start, exon.end, exon.strand, exon.gene_name, exon.gene_id, position, transcript_id, hg_version)
